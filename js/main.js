...
function iniciarVideoIntro(){
  ...
  function encerrar(){
    overlay.style.transition='opacity .8s ease'; overlay.style.opacity='0';
    setTimeout(()=>{overlay.remove();inicializar();},850);
  }
  video.addEventListener('ended',encerrar,{once:true});
  skipBtn.addEventListener('click',encerrar,{once:true});
  overlay.addEventListener('click',encerrar,{once:true});
  let vis=true;
  setInterval(()=>{const p=document.getElementById('video-pulso');if(p)p.style.opacity=vis?'1':'0.2';vis=!vis;},700);
  video.onerror=()=>setTimeout(encerrar,300);
  setTimeout(()=>{if(overlay.isConnected)encerrar();},30000);
}
...
function registrarEventos(){
  ...
  document.getElementById('btn-parar').onclick=()=>{
    solicitarParada=true; cmdMover=null; cmdGirar=null; aguardandoPasso=false;
  ...
  document.getElementById('btn-passo').onclick=()=>{
    ...
    if(estadoExecucao==='parado'||estadoExecucao==='concluido'){
      ...
      const r=resolverStep; resolverStep=null; _resetStep(); r();
    }
  };
  ...
  document.querySelectorAll('.aba-fase').forEach(btn=>{
    btn.onclick=()=>{solicitarParada=true;if(workspaceBlockly)workspaceBlockly.highlightBlock(null);
      setTimeout(()=>{limparEspaco();carregarFase(+btn.dataset.fase);},80);};
  });
  ...
  document.getElementById('btn-proxima-fase').onclick=()=>{
    if(faseAtual<5){
      ...
      setTimeout(()=>{limparEspaco();carregarFase(faseAtual+1);},80);
    }
  };
}
